...
```bash
sudo openssl genrsa -out /cert/registry.key 2048
```

- registry.csr 생성

```bash
sudo openssl req -new -key /cert/registry.key -subj /CN=10.102.20.32 -out /cert/registry.csr
```

- extfile.cnf 생성

```ini
subjectAltName = IP:10.102.20.32
```
...
❯ sudo openssl x509 -req \
  ...
  -days 10000 \
  -sha512 \
  -extfile registry_ext.cnf

Certificate request self-signature ok
subject=CN = 10.101.20.32
```

- registry 유저 및 비밀번호 설정
...
[compose 설정](./registry_docker-compose.yml)
\+
[nginx 설정](./registry_nginx.yml)
혹은
[nginx 없이 설정](./registry_wo_nginx_docker-compose.yml)
...
모든 레지스트리가 TLS를 하려면 무조건 필요한 작업이다!

각 docker 환경 (mac은colima)에 전달해야만 인증 및 작업이 가능하다.

```bash
sudo cp /cert/ca.crt /etc/docker/certs.d/10.102.20.32/ca.crt
sudo chmod 644 /etc/docker/certs.d/10.102.20.32/ca.crt
sudo cp /etc/docker/certs.d/10.102.20.32/ca.crt /usr/local/share/ca-certificates/registry-ca.crt
sudo update-ca-certificates
